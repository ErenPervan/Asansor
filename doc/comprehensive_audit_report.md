...
Date: 2026-06-07
Revised: 2026-06-07 (post-review corrections applied)
...
- `flutter test`: timed out after 5 minutes.
- `flutter test test/core/services/sync_queue_service_test.dart --reporter expanded`: timed out after 2 minutes.
...
Justification: I would not approve this project for production deployment or investor review in its current state. The feature surface is promising, but the failure modes are too serious for a field-service product. A technician can lose unsynced data, notification privileges are too broad, credentials have been committed, release signing is not production-grade, and the test suite currently cannot be trusted because it hangs.
...
Description: Faults are queued only when `isOnlineProvider` says offline (`lib/features/fault/providers/fault_providers.dart:103-131`). If the app thinks it is online and the Supabase write fails, the operation goes through `AsyncValue.guard` without a durable fallback (`lib/features/fault/providers/fault_providers.dart:135-143`).

Impact: A technician in a basement or machine room with unstable connectivity can lose a fault report.

Recommended Solution: Make fault reporting queue-first. Persist the report and media locally before attempting remote upload. Treat transient network/Supabase failures as queueable.
...
Recommended Solution: Rotate/revoke the service account immediately, purge secrets from git history using `git filter-repo` or BFG, verify `.env.local` is no longer tracked (`git rm --cached`), use Supabase/Firebase secret stores, and add CI secret scanning.
...
- The full test suite times out.
- A single service test file also times out.
...
1. Fix test hang first.
2. Add fake Supabase clients and deterministic queue tests.
3. Add offline-first integration tests.
4. Add RLS migration tests.
5. Add Edge Function authorization tests.
6. Add CI coverage thresholds.
7. Add golden tests for major screens, not only shared states.
...
1. Rotate and remove committed Firebase service account secrets.
2. Stop automatic deletion of sync queue/local writes.
3. Make fault and maintenance writes queue-first.
...
9. Make `flutter test` complete reliably.
10. Remove debug token logging as defense-in-depth (low production risk since `debugPrint`/`kDebugMode` are stripped in release builds).
11. Fix webhook trigger Authorization header inconsistency between `notify_technician_on_assignment` and `notify_fault_report`.
12. Replace `<YOUR_ANON_KEY>` placeholder in `notify_technician_on_assignment` migration.
13. Add timeout guards to online photo upload path in `MaintenanceController._uploadPhotos`.
