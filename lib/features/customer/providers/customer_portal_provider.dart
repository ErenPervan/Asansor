...
final customerElevatorProvider = FutureProvider.autoDispose<ElevatorModel?>((ref) async {
...
final customerMaintenanceLogsProvider = FutureProvider.autoDispose<List<MaintenanceLogModel>>((ref) async {
  final profile = await ref.watch(currentProfileProvider.future);
  final elevatorId = profile?.elevatorId;
  ...
  return response.map((json) => MaintenanceLogModel.fromJson(json)).toList();
});
