...
''';
  ...
  final files = {
    'elevator_detail_header.dart': {
      ...
      'publics': ['ElevatorDetailHeader', 'DetailMetaCell', 'DetailStatusBadge'],
      'end': '_ActionButtons'
    },
    ...
    'elevator_system_monitor.dart': {
      'classes': ['_SystemMonitorSection', '_NextMaintenanceContent', '_StatusIndicator', '_StatChip'],
      'publics': ['SystemMonitorSection', 'NextMaintenanceContent', 'SystemStatusIndicator', 'SystemStatChip'],
      'end': '_MaintenanceHistorySection'
    },
    'elevator_maintenance_history.dart': {
      'classes': ['_MaintenanceHistorySection', '_MaintenanceHistorySectionState', '_TimelineItem', '_TimelineCard', '_Chip'],
      'publics': ['MaintenanceHistorySection', 'MaintenanceHistorySectionState', 'TimelineItem', 'TimelineCard', 'StatusChip'],
      'end': '_BottomNav'
    },
  ...
  if (startIdx != -1 && endIdx != -1 && startSheet != -1 && endSheet != -1) {
    var newDetailView = [...lines.sublist(0, startIdx), ...lines.sublist(endIdx, startSheet), ...lines.sublist(endSheet)].join('\n');
    ...
    for (final config in files.values) {
      ...
      for (int i = 0; i < classes.length; i++) {
        ...
        newDetailView = newDetailView.replaceAll('$cls.', '$publicCls.'); // e.g. .new or .fromJson
        newDetailView = newDetailView.replaceAll('<$cls>', '<$publicCls>'); 
      }
    }
    
    final imports = files.keys.map((f) => "import '../widgets/detail/$f';").join('\n');
    ...
    newDetailLines.insert(lastImportIdx + 2, "import '../../../core/widgets/app_bottom_nav_bar.dart';");
    ...
    finalDetailView = finalDetailView.replaceAll('const _BottomNav()', 'const AppBottomNavBar(currentIndex: -1)');
    finalDetailView = finalDetailView.replaceAll('class _BottomNav', '// class _BottomNav');
    finalDetailView = finalDetailView.replaceAll('class _NavItem', '// class _NavItem');
