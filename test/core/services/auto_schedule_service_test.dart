...
void main() {
  ...
  group('AutoScheduleService Tests', () {
    test(
      'generateMonthlyMaintenances returns 0 when no elevators exist',
      () async {
        when(
          () => mockElevatorRepo.getAllElevators(),
        ).thenAnswer((_) async => []);

        final result = await service.generateMonthlyMaintenances(
          DateTime(2026, 5),
        );
    ...
    test(
      'generateMonthlyMaintenances skips elevators without maintenanceDay',
      () async {
        final elevators = [
          const ElevatorModel(
            id: 'e1',
    ...
    test(
      'generateMonthlyMaintenances skips already scheduled elevators',
      () async {
        final elevators = [
          const ElevatorModel(
            id: 'e1',
        ...
        when(
          () => mockScheduleRepo.getScheduledElevatorIdsForMonth(any()),
    ...
    test(
      'generateMonthlyMaintenances inserts tasks for eligible elevators',
      () async {
        final elevators = [
          const ElevatorModel(
            id: 'e1',
        ...
        when(
          () => mockScheduleRepo.bulkInsertPeriodicSchedules(any()),
