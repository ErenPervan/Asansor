  ...
  group('ProfileModel Tests', () {
    ...
    group('displayName', () {
      ...
      test('returns email prefix if fullName is empty/null', () {
        final model = TestFactories.createProfile(fullName: null, email: 'veli.can@test.com');
        expect(model.displayName, 'veli.can');
      });

      test('returns id prefix if both fullName and email are empty/null', () {
        final model = TestFactories.createProfile(fullName: null, email: null, id: '1234567890');
        expect(model.displayName, '12345678');
      });

      test('returns full id if id is short', () {
        final model = TestFactories.createProfile(fullName: null, email: null, id: '123');
    ...
    group('initials', () {
      test('returns initials for 2+ words', () {
        ...
        final model3 = TestFactories.createProfile(fullName: 'Ahmet Can Yılmaz');
      ...
      test('returns fallback if no name', () {
         final model = TestFactories.createProfile(fullName: null, email: 'user@test.com');
    ...
    group('roleTr', () {
      test('translates roles correctly', () {
        ...
        expect(TestFactories.createProfile(role: 'technician').roleTr, 'Teknisyen');
        expect(TestFactories.createProfile(role: 'customer').roleTr, 'Müşteri');
        expect(TestFactories.createProfile(role: 'unknown').roleTr, 'unknown'); // Fallback
      });
    });
    ...
    group('role booleans', () {
      test('isAdmin', () {
        ...
        expect(TestFactories.createProfile(role: 'technician').isAdmin, isFalse);
      });

      test('isTechnician', () {
        expect(TestFactories.createProfile(role: 'technician').isTechnician, isTrue);
        expect(TestFactories.createProfile(role: 'customer').isTechnician, isFalse);
      });

      test('isCustomer', () {
        expect(TestFactories.createProfile(role: 'customer').isCustomer, isTrue);
        expect(TestFactories.createProfile(role: 'admin').isCustomer, isFalse);
      });
    });

    test('copyWith updates fields', () {
      final original = TestFactories.createProfile(id: '1', role: 'technician', elevatorId: 'e1');
